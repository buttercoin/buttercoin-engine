...
class BookStore
  ...
  for_levels_above: (price, cb) =>
    cursor = @tree.range(new Amount('0'), price)
    running = true
...
module.exports = class Book
  ...
  fill_orders_with: (order) =>
    ...
    #cur = @store.head()
    closed = []
    amount_filled = Amount.zero
    amount_remaining = order.received_amount.clone()
    amount_spent = Amount.zero
    ...
    @store.for_levels_above order.price, (price, order_level) =>
      ...
      if order_level.size.lte(amount_remaining)
        ...
        amount_spent = amount_spent.add(order_level.size.multiply(price))
        ...
        while cur_order?.offered_amount.lte(amount_remaining)
          ...
          amount_spent = amount_spent.add(cur_order.offered_amount.multiply(price))
        ...
        if amount_remaining.eq(Amount.zero)
          ...
          amount_spent = amount_spent.add(amount_remaining.multiply(price))
    ...
    order.price = order.offered_amount.divide(order.received_amount)
    if amount_remaining.is_zero()
      ...
      remaining = new Order(order.account,
                            order.offered_currency,
                            orig_order.price.inverse().multiply(amount_remaining),
                            order.received_currency,
                            orig_order.received_amount.subtract(amount_filled))
